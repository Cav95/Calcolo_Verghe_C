...
char *gen_masc(int l_arr)
{
    ...
    array = ( char*)malloc(sizeof(char)*l_arr);
...
void calc_tub(Codice *lista)
{
    ...
    FILE* fs; /*puntatore a file esterno*/
    ...
    fs = fopen("Output_verghe.txt", "w+");/*apertura file esterno*/
    temp = lista;
    ...
    while ( temp != NULL)
    {
        ...
        while ( controllo == 1)
        {
            ord_tagl = tagli(temp->lista,2000,temp->indice); /* estrazione ordine di taglio*/
            ...
            resto = 6000; /*imposto la lunghezza della verga , si pu� rendere dinamico scegliendo il tipo tubolare*/
        ...
        printf("Numero verghe (%s)= %d\n", temp->code,num_ver);
        fprintf(fs, "Numero verghe (%s)= %d\n", temp->code, num_ver);
...
char *tagli (Verga *ver, int ricor , int l_arr)
{
    ...
    do
    {
        ...
        for ( i = 0 ; i < l_arr ; i++)
        {
            ...
            if (v_2[i] == 1 && ver[i].qt == 0) /*lo azzero se le quantit� sono a zero */
            {
                v_2[i] = 0;
            }
